...
jmb.add(fileMenu);
LookMenu.createLookAndFeelMenuItem(fileMenu,this);
setVisible(true);
}
...
tmp.add(rbm[i]);
bg.add(rbm[i]);
rbm[i].addActionListener(new ACListeners(infos[i].getClassName(),cmp));
}
...
class ACListeners implements ActionListener
{
String classname;
Component jf;

ACListeners(String cln,Component jf)
{
this.jf=jf;
